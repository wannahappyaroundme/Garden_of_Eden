...
class GeminiService:
    ...
    async def generate_response_stream(
        ...
        """
        Generate AI response using streaming for faster perceived speed

        Yields:
            str: Text chunks as they are generated by Gemini
        """
        try:
            # Build Master Directive (same as non-streaming version)
            master_directive = self._build_master_directive(
                user_profile=user_profile,
                persona=persona,
                pitfall_warning_mode=pitfall_warning_mode,
            ...
            # Build context
            context = self._format_recent_conversations(recent_conversations)
            ...
            if camera_frames and len(camera_frames) > 0:
                content_parts.append("\n\n### Visual Context (captured frames):")
                for i, frame in enumerate(camera_frames[:3]):  # Max 3 frames
                    content_parts.append(frame)
                    content_parts.append(f"[Frame {i+1} captured during conversation]")

            logger.info(f"🌊 Starting streaming response generation for user message: {user_message[:100]}...")
